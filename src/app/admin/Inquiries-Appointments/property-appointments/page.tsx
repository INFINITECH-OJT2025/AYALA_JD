...
export default function AdminAppointments() {
  ...
  useEffect(() => {
    if (filter === "all") {
      ...
      setFilteredAppointments(appointments.filter((inq) => inq.status === "archived"));
    } else {
      setFilteredAppointments(appointments.filter((inq) => inq.status !== "archived"));
    }
  }, [filter, appointments]);
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setAppointments((prev) =>
        prev.map((appt) =>
          appt.id === selectedAppointment.id ? { ...appt, status: "replied" } : appt
        )
      );
...

    {
      ...
      cell: ({ row }) => (
        <div className="flex space-x-2">
          <Button size="sm" variant="outline" onClick={() => setSelectedAppointment(row.original)}>
  ...
  return (
    <div className="bg-white dark:bg-black p-6 w-full rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-4 text-center md:text-left">Property Appointments</h2>
      ...
      {loading ? (
        <div className="flex justify-center items-center">
          <p className="text-gray-500 dark:text-gray-300">Loading appointments...</p>
        </div>
      ) : error ? (
            ...
            <div className="mb-2 p-3 border rounded bg-gray-100 dark:bg-gray-800">
                ...
                {selectedAppointment.property?.property_image ? (
                    (() => {
                      ...
                      const images = Array.isArray(selectedAppointment.property.property_image)
                        ? selectedAppointment.property.property_image
                ...
                <b>Property Name: </b>
                    {selectedAppointment.property?.property_name || "Unknown Property"}
                </p>
                ...
                <p className="text-gray-700 dark:text-gray-300 mt-1">
                    ...
                    ? new Date(`1970-01-01T${selectedAppointment.time}`).toLocaleTimeString("en-US", {
                        hour: "2-digit",
                        minute: "2-digit",
                        hour12: true,
                        })
