...
import { fetchApplicants, approveApplicant, rejectApplicant, deleteApplicant } from "@/lib/api";
...
export default function JobApplicants() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error("Failed to delete applicant", {
        description: "There was an issue deleting the applicant. Please try again.",
      });
    } finally {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    { accessorKey: "status", header: "Status" }, // ✅ Show application status
    {
      ...
      header: "Resume",
      cell: ({ row }) => (
        <Button size="sm" variant="outline" onClick={() => window.open(row.original.resume_path, "_blank")}>
        ...
        return (
          <div className="flex space-x-2">
            ...
            <Button 
              size="sm" 
              variant="outline" 
              onClick={() => { setSelectedApplicant(row.original); setActionType("approve"); }}
              disabled={status === "approved"} // ✅ Disable if already approved
            >
              <Check className="w-4 h-4 text-green-600" /> {status === "approved" ? "Approved" : "Approve"}
            </Button>
            ...
            <Button 
              size="sm" 
              variant="destructive" 
              onClick={() => { setSelectedApplicant(row.original); setActionType("reject"); }}
              disabled={status === "rejected"} // ✅ Disable if already rejected
            >
              <X className="w-4 h-4" /> {status === "rejected" ? "Rejected" : "Reject"}
            </Button>
  ...
  return (
    <div className="bg-white dark:bg-black p-6 w-full rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-4 text-center md:text-left">Job Applicants</h2>
      ...
      {loading ? (
        <div className="flex justify-center items-center">
          <p className="text-gray-500 dark:text-gray-300">Loading applicants...</p>
        </div>
      ) : error ? (
      ...
      {selectedApplicant && actionType && (
        <Dialog open={!!selectedApplicant} onOpenChange={() => setSelectedApplicant(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{actionType === "approve" ? "Approve Applicant" : "Reject Applicant"}</DialogTitle>
            </DialogHeader>
            <div>
              <p className="mb-2">Send a message to <b>{selectedApplicant.email}</b>:</p>
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button onClick={handleActionSubmit} variant={actionType === "approve" ? "success" : "destructive"}>
                {actionType === "approve" ? "Send Approval Email" : "Send Rejection Email"}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
