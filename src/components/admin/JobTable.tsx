...
export default function JobTable() {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setJobs(jobs.map((job) => (job.id === editJob.id ? { ...job, ...editForm } : job)));
      setEditJob(null);
  ...
  return (
    <div className="container mx-auto">
      ...
      <h2 className="text-2xl font-bold mb-4 text-center md:text-left">Job Listings</h2>
        ...
        <Dialog open={!!editJob} onOpenChange={() => setEditJob(null)}>
          <DialogContent>
            ...
            <form onSubmit={handleUpdate} className="space-y-2" method="POST">
              {/* Title */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Job Title</label>
                  <Input name="title" value={editForm.title || ""} onChange={handleEditChange} required />
                </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {/* Location */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Location</label>
                  <Input name="location" value={editForm.location || ""} onChange={handleEditChange} required />
                </div>

                {/* Job Type */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Job Type</label>
                  <Input name="category" value={editForm.type || ""} onChange={handleEditChange} required />
                </div>

                {/* Job Slots */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Job Slots</label>
                  <Input name="slots" value={editForm.slots || ""} onChange={handleEditChange} required />
                </div>

                {/* Category */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Category</label>
                  <Input name="category" value={editForm.category || ""} onChange={handleEditChange} required />
                </div>

                {/* Salary */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Salary</label>
                  <Input name="salary" value={editForm.salary || ""} onChange={handleEditChange} />
                </div>

                {/* Deadline */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Application Deadline</label>
                  <Input name="deadline" type="date" value={editForm.deadline || ""} onChange={handleEditChange} />
                </div>
              </div>
              ...
              <div className="space-y-4">
                {/* Description */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Job Description</label>
                  <Textarea name="description" value={editForm.description || ""} onChange={handleEditChange} required />
                </div>

                {/* Image Upload */}
                <div>
                  <label className="block text-sm font-bold dark:text-gray-100 text-gray-700">Job Image</label>

                  {/* Show the existing image or the newly selected preview */}
                  {editForm.image_url && !editForm.imagePreview && (
                    <img src={editForm.image_url} alt="Current Image" className="w-32 h-32 object-cover rounded-lg mb-2" />
                  )}

                  {editForm.imagePreview && (
                    <img src={editForm.imagePreview} alt="New Image Preview" className="w-32 h-32 object-cover rounded-lg mb-2" />
                  )}
